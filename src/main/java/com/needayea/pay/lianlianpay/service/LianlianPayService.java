...
@Service("LianLianPay")
public class LianlianPayService implements IPayment {
    ...
    public String pay(PayRequest payRequest) {
        ...
        paymentInfo.setNo_order(payRequest.getOrderId());
        paymentInfo.setDt_order(sdf.format(new Date()));
        paymentInfo.setMoney_order(payRequest.getTotalAmount().toString());
        ...
        //web
        paymentInfo.setFlag_chnl("2");
        paymentInfo.setName_goods(payRequest.getGoodsName());
        paymentInfo.setInfo_order(payRequest.getInfo());
        // 单位分钟，可以为空，默认7天
        ...
        String url = payDataBean.getString("gateway_url");
